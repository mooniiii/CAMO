...
<%@ include file="/WEB-INF/views/includes/headerBoard.jsp" %>
    ...
    <div class="board">
                <h2><a href="/board/cm_evnet/list">EVENT</a></h2>
                <input type="hidden" name="board_name" value="ev">
                    <div class="registerBtn">
                        <button><a href="/board/cm_event/register">글쓰기</a></button>
                    </div>
                    <div class="boardList">
                        ...
                        <ul>
                        	<c:forEach var="boardlist" items="${list}">
	                            <li>
	                                ...
	                                <p class="title"><a href="/board/cm_event/read?bno=${boardlist.bno}">${boardlist.title}</a>[${boardlist.replyCnt}]</p>
                    ...
                    <div class="search">
						<div class="searchIn">
							<form id="searchForm" action="/board/cm_event/list" method="get">
                    ...
                    <div class="pager">
                            <ul>
                            	<c:if test="${pageMaker.prev}">
                            		<li><a href="/board/cm_event/list?pageNum=${pageMaker.startPage - 1}&amount=${pageMaker.cri.amount}" class="prev"></a></li>
                            	</c:if>
                            	<c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
	                                <li class="${pageMaker.cri.pageNum==num?'active':''}">
	                                	<a href="/board/cm_event/list?pageNum=${num}&amount=${pageMaker.cri.amount}">${num}</a>
	                                </li>
                                </c:forEach>
                                <c:if test="${pageMaker.next}">
                                	<li><a href="/board/cm_event/list?pageNum=${pageMaker.endPage + 1}&amount=${pageMaker.cri.amount}" class="next"></a></li>
                                </c:if>
                            </ul>
                    </div> <!--.Pager-->
            </div> <!--.board-->
